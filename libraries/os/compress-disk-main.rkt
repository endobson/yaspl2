...
;(struct output-buffer (bytes [pos #:mutable] port))
;
;(define (output-buffer/increment-pos o x)
;  (set-output-buffer-pos!
;    o
;    (fxmodulo (fx+ (output-buffer-pos o) x)
;              (bytes-length (output-buffer-bytes o))))
;  (when (zero? (output-buffer-pos o))
;    (write-all-bytes (output-buffer-bytes o) (output-buffer-port o))))
;
;
;(define (output-buffer/handle-literal o val)
;  (bytes-set! (output-buffer-bytes o) (output-buffer-pos o) val)
;  (output-buffer/increment-pos o 1))
;
;(define (output-buffer/handle-repeat o len distance)
;  (unless (fx= 0 len)
;    (match-define (output-buffer bytes pos _) o)
;    (define raw-start (fx- pos distance))
;    (define-values (start copy-len)
;      (if (fx< raw-start 0)
;          (values (fx+ (bytes-length bytes) raw-start)
;                  (fxmin len (fx- raw-start)))
;          (values raw-start (fxmin len distance (fx- (bytes-length bytes) pos)))))
;
;    (bytes-copy! bytes pos bytes start (fx+ start copy-len))
;        
;    (output-buffer/increment-pos o copy-len)
;    (output-buffer/handle-repeat o (fx- len copy-len) distance)))
;
;(define (output-buffer/finalize-output o)
;  (match-define (output-buffer bytes pos port) o)
;  (write-all-bytes (subbytes bytes 0 pos) port)
